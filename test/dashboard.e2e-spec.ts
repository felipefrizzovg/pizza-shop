...
test('display month orders amount metric', async ({ page }) => {
  ...
  expect(page.getByText('+ 7% em ao mês passado')).toBeVisible()
})
...
test('display month cancelled orders amount metric', async ({ page }) => {
  ...
  expect(page.getByText('-5% em ao mês passado')).toBeVisible()
})
...
test('display month revenue metric', async ({ page }) => {
  ...
  expect(page.getByText('-10% em ao mês passado')).toBeVisible()
})
